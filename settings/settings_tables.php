 ...
 *             		Eduardo Aguirrebeña Zapata (eaguirrebena@alumnos.uai.cl)
...
// Used in resources.php it generates table with all existing resource.
	...
	public static function myReservations($userid = null){
		...
		foreach ($reservations as $reservation) {
			...
			//check if the time to confirm is in the range
			else if($actualtimeanddate < $timeafterdateandhour && $actualtimeanddate > $timebeforedateandhour){ 
			...
			}else if($actualtimeanddate>$timeafterdateandhour && $reservation->confirm == 0){
